...
void ModSpeedometer() {
	...
	if (!Util::SRMM_GetSetting(SRMM_SPEEDOMETER_INCLUDE_Z)) {
		// overwrite to only include x&y axis
		Util::WriteBytes((void*)position1, 0x12, 1);
		Util::WriteBytes((void*)position2, 0x12, 1);
	}
	else {
		// overwrite back to default values
		Util::WriteBytes((void*)position1, 0x10, 1);
		Util::WriteBytes((void*)position2, 0x10, 1);
	}

	// Enable/Disable Fadeout
	if (!Util::SRMM_GetSetting(SRMM_SPEEDOMETER_FADEOUT)) {
		// disable fadeout
		Util::WriteBytes((void*)alwaysShow, 0x90, 2);
	}
	else {
		// overwrite back to default values
		Util::WriteBytes((void*)alwaysShow, 0x72, 1);
		Util::WriteBytes((void*)(alwaysShow + 0x1), 0x2C, 1);
	}
}
...
void ModAltTab() {
	...
	Util::WriteBytes((void*)target, 0x75, 1);
}
...
void __fastcall detourUpdateLoadingScreenProgress(long long var1) {
	...
	if (Util::FindDMAAddy((uintptr_t)GetModuleHandle("vgui2.dll") + 0x122268, { 0x40, 0x720, 0x2A0 }, progressAddr)) {
...
DWORD WINAPI Thread(HMODULE hModule) {
	...
	while (true) {
		...
		if (!hooksEnabled && Util::SRMM_GetSetting(SRMM_CK_FIX)) {
			enableInputHooks();
		}
		if (hooksEnabled && !Util::SRMM_GetSetting(SRMM_CK_FIX)) {
			disableInputHooks();
